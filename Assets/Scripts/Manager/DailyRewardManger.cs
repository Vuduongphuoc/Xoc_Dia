...
////Manager to manage all daily login
//public static class DailyRewardManger
//{
...
//    private static bool _isInFirstCycle => totalDays / _setting.cycleDays == 0;

//    //Total days passed since the first day (miss reward if not login), or the total days that obtained rewards.

//    public static int totalDays => _settings.missRewardIfNotLogin ? (DateTime.Now.Date - _startDateTime.Date).Days
//        : (_dailyLoginCount - (isTodayRewardObtained ? 1: 0));
...
//        {
//            _startDateTime = DateTimeUtils.GetPlayerPrefs(PREFS_START_DATE);
//            if (_startDateTime.AddDays(1) <= DateTime.Now.Date)
